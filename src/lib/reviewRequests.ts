...
export async function sendReviewRequestEmail(input: { to: string; clientName?: string; reviewUrl: string }) {
  ...
  const subject = "A quick Civil At Hand review would help";
  const text = `Hi ${name},\n\nThank you for working with Civil At Hand. If you are comfortable sharing your experience, a short Google review helps future clients understand how the consultancy works.\n\n${reviewUrl}\n\nThank you,\nCivil At Hand`;
  const html = `<div style="font-family:Arial,sans-serif;max-width:640px;margin:auto;line-height:1.6;color:#0f172a"><h1 style="color:#f97316">Civil At Hand</h1><p>Hi ${escapeHtml(name)},</p><p>Thank you for working with Civil At Hand. If you are comfortable sharing your experience, a short Google review helps future clients understand how the consultancy works.</p><p><a href="${escapeHtml(reviewUrl)}" style="display:inline-block;background:#f97316;color:#fff;padding:12px 18px;border-radius:10px;text-decoration:none;font-weight:700">Leave a Google review</a></p><p>Thank you,<br/>Civil At Hand</p></div>`;
  await transporter.sendMail({ from: `"Civil At Hand" <${FROM}>`, to: input.to, subject, text, html });
  return { sent: true };
}
