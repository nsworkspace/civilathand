...
export default function VendorRegisterPage() {
  ...
  const openReview = () => {
    ...
    if (!authUser?.emailVerified) return setError("Please sign in with a verified Civil At Hand account before continuing.");
  ...
  const submit = async (event?: React.FormEvent) => {
    ...
    if (!authUser?.emailVerified) return setError("Please sign in with a verified Civil At Hand account before submitting.");
  ...
  if (success) return <main className="min-h-screen bg-slate-950 p-4 flex items-center justify-center"><div className="w-full max-w-xl rounded-[2rem] bg-white p-9 text-center shadow-2xl"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-50"><CheckCircle className="h-9 w-9 text-emerald-600" /></div><p className="mt-6 text-xs font-black uppercase tracking-[.22em] text-orange-600">Civil At Hand Vendor Network</p><h1 className="mt-2 text-3xl font-black text-slate-950">Profile Submitted for Review</h1><p className="mt-4 text-sm leading-7 text-slate-600">Thank you for registering. Your business information has been securely recorded. Approval is handled by Civil At Hand before your profile is published in the vendor network.</p><div className="mt-7 grid gap-3 sm:grid-cols-2"><a href="/vendors" className="rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white hover:bg-orange-600">Browse Vendor Network</a><a href="/" className="rounded-xl border border-slate-200 px-5 py-3 text-sm font-black text-slate-700 hover:bg-slate-50">Back to Civil At Hand</a></div></div></main>;

  return <main className="min-h-screen w-full max-w-full overflow-x-hidden bg-[#f7f8fb] text-slate-900">
    <section className="relative w-full overflow-hidden bg-slate-950 px-4 py-10 text-white sm:px-6 sm:py-14 md:py-20"><div className="absolute inset-0 opacity-30" style={{ backgroundImage: "radial-gradient(circle at 12% 20%,#f97316 0,transparent 25%),radial-gradient(circle at 90% 80%,#475569 0,transparent 28%)" }} /><div className="relative mx-auto w-full max-w-6xl min-w-0"><div className="flex flex-wrap items-center justify-between gap-4"><div className="inline-flex items-center gap-2 rounded-full border border-orange-400/25 bg-orange-500/10 px-4 py-2 text-xs font-black uppercase tracking-[.18em] text-orange-300"><BadgeCheck size={15} /> Civil At Hand Vendor Network</div><a href="/vendors" className="rounded-xl border border-white/15 bg-white/5 px-4 py-2.5 text-xs font-black text-slate-200 hover:bg-white/10">Explore Approved Vendors</a></div><div className="mt-7 max-w-4xl min-w-0"><h1 className="break-words text-3xl font-black leading-[1.08] tracking-tight sm:text-4xl md:text-6xl">Register your business in a few simple steps.</h1><p className="mt-4 max-w-3xl text-sm leading-6 text-slate-300 sm:text-base sm:leading-7 md:text-lg md:leading-8">Tell us who you are, what work you do and where you work. We will guide you through the rest.</p></div><div className="mt-8 grid gap-3 sm:grid-cols-3"><HeroPoint icon={<SearchCheck />} title="Be found" text="Your approved business profile can be found by people looking for your type of work." /><HeroPoint icon={<ShieldCheck />} title="Your details stay private" text="Your direct phone, email and full address are not shown on the public vendor profile." /><HeroPoint icon={<Users />} title="Relevant work" text="When a suitable requirement comes in, Civil At Hand may connect you with the opportunity." /></div></div></section>
